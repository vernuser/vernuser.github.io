...
/** 构建期已解析的配置缓存：静态导出时避免运行时读盘导致路由被判为动态 */
let fileConfigCache: AppConfig | null = null;
function readFileConfig(): AppConfig | null {
  if (fileConfigCache) return fileConfigCache;
  const configPath = join(CONFIG_DIR, FILE_NAME);
  if (!existsSync(configPath)) return null;
  try {
    fileConfigCache = JSON.parse(readFileSync(configPath, "utf-8")) as AppConfig;
