...
.brand{display:flex;align-items:center;gap:9px;font-weight:700;font-size:17px;color:#fff;text-decoration:none;white-space:nowrap}
.brand svg{width:22px;height:22px;flex:none}
.menu{display:flex;align-items:center;gap:26px;list-style:none;margin:0;padding:0}
.menu a{color:rgba(255,255,255,.86);text-decoration:none;font-size:15px;transition:opacity .25s ease,color .25s ease}
.menu a:hover{color:#fff;opacity:.85}
.menu a.active{color:#fff;font-weight:600}
.topbar .right{display:flex;align-items:center;gap:14px}
.icon-btn{
  width:34px;height:34px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;
  border:1px solid var(--line);background:rgba(255,255,255,.1);color:#fff;cursor:pointer;font-size:15px;
  transition:background .25s ease,transform .25s ease;
}
.icon-btn:hover{background:rgba(58,163,227,.45);transform:translateY(-2px)}

/* 主体 */
.layout{max-width:1180px;margin:0 auto;padding:26px 20px 90px;display:flex;gap:22px;align-items:flex-start}
.side{flex:0 0 250px;width:250px;position:sticky;top:86px;max-height:calc(100vh - 108px);overflow-y:auto;overflow-x:hidden;scrollbar-width:thin}
...
.post{
  position:relative;display:flex;flex-direction:column;height:246px;overflow:hidden;text-decoration:none;
...
.post .cover{position:absolute;inset:0;background:linear-gradient(135deg,rgba(58,163,227,.5),rgba(98,163,61,.5));transform:scale(1.02);transition:transform .8s cubic-bezier(.22,.61,.36,1);overflow:hidden}
.post .cover img{width:100%;height:100%;object-fit:cover;display:block}
.post:hover .cover{transform:scale(1.07)}
.post .veil{position:absolute;inset:0;background:linear-gradient(to top,rgba(6,18,32,.94) 0%,rgba(6,18,32,.5) 46%,rgba(6,18,32,.12) 100%)}
.post .body{position:relative;z-index:2;margin-top:auto;padding:18px 20px 20px}
.post .meta{display:flex;align-items:center;gap:10px;margin-bottom:8px;font-size:11.5px}
.post .chip{padding:1px 9px;border-radius:999px;background:rgba(255,255,255,.18);border:1px solid rgba(255,255,255,.26);color:#fff}
.post .date{color:rgba(255,255,255,.72)}
.post h2{margin:0;font-size:18px;line-height:1.45;font-weight:700;color:#fff;text-shadow:0 2px 12px rgba(4,14,26,.7);
  display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
.post p{margin:7px 0 0;font-size:12.5px;line-height:1.65;color:rgba(255,255,255,.78);
  display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
.post:hover{z-index:5;transform:translateY(-5px) scale(1.02);box-shadow:0 26px 54px rgba(6,20,36,.44)}
.feed.hovering .post:not(:hover){transform:scale(.97);filter:blur(1.2px)}
...
function feedBlock(articles) {
  const cards = articles
    .map((a) => `<a class="post" href="${a.url}" title="${esc(a.title)}">
  <span class="cover">${a.cover ? `<img src="${esc(a.cover)}" alt="" loading="lazy" onerror="this.remove()">` : ''}</span>
  ...
  <div class="body">
    ...
    .join('\n');
  return `<div class="feed" id="feed">${cards}</div>`;
}
...
<header class="topbar">
  <div class="topbar-in">
    <a class="brand" href="/">${ICON_LOGO}<span>${esc(SITE.name)}</span></a>
    <nav>
      <ul class="menu">
    ...
    <div class="right">
      <button class="icon-btn" id="theme" type="button" title="切换深浅色" aria-label="切换深浅色">
        <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round"><circle cx="12" cy="12" r="4.2"/><path d="M12 2.6v2.2M12 19.2v2.2M2.6 12h2.2M19.2 12h2.2M5.4 5.4l1.6 1.6M17 17l1.6 1.6M18.6 5.4 17 7M7 17l-1.6 1.6"/></svg>
      </button>
    </div>
  </div>
</header>
...
const articles = await filterCovers(readArticles());
if (!articles.length) {
